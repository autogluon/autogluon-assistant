...
class TutorialIndexer:
    ...
    def search(
        ...
        if (tool_name not in self.indices or 
            tutorial_type not in self.indices[tool_name]):
    ...
    def get_all_summaries(self, tool_name: str, condensed: bool = False) -> List[Dict]:
        ...
        if (tool_name not in self.metadata or 
            tutorial_type not in self.metadata[tool_name]):
            return []
