...
def messages(log_entries: List[Dict], max_iter: int):
    ...
    if state["current_phase"]:
        stage_text.markdown(f"### {state['current_phase']}")
    
    # 计算并显示进度
    progress = _calculate_progress(state, total_stages, max_iter)
    progress_bar.progress(progress)
    
    # 渲染所有已知阶段（按顺序）
    rendered_phases = set()
    phase_order = ["Reading"] + [f"Iteration {i+1}" for i in range(max_iter)] + ["Output"]
...
def _calculate_progress(state: Dict, total_stages: int, max_iter: int) -> float:
    ...
    if state["current_phase"] == "Reading":
        ...
        try:
            ...
            return idx / total_stages
        except:
            pass
    
    # 根据已完成的阶段计算
    completed = sum(1 for phase_data in state["phase_states"].values() 
