...
class NodeTree(Flowable):
    ...
    def _draw_node(self, node_id: int, x: float, y: float):
        ...
        self.canv.linkURL(
            f"#node_{node_id}", 
            (x - radius - click_padding, y - radius - click_padding, 
             x + radius + click_padding, y + radius + click_padding), 
            relative=0
        )
...
class NodeVisualizer:
    ...
    def _create_node_summary(self, node: Node) -> List[Flowable]:
        ...
        elements.append(Paragraph(
            f'<a name="node_{node.id}"></a>',
            self.styles["Normal"]
        ))
        
        # Node title
        elements.append(Paragraph(
            f"Node {node.id} ({node.stage})",
            self.styles["NodeTitle"]
        ))
        ...
        special_attrs = {'parent', 'children', 'error_message', 'error_analysis', '_lock', 'python_code', 'bash_script', 'stdout', 'stderr', 'tutorial_retrieval', 'tutorial_prompt'}
        ...
        if not node.is_successful and node.error_message:
            ...
            if len(truncated_message) > 2000:
                truncated_message = truncated_message[:2000] + f"... [truncated, {len(node.error_message)} chars total]"
        ...
        if node.error_analysis:
            ...
            if len(truncated_analysis) > 2000:
                truncated_analysis = truncated_analysis[:2000] + f"... [truncated, {len(node.error_analysis)} chars total]"
                
            elements.append(Paragraph(
                truncated_analysis.replace("\n", "<br/>"),
                self.styles["Normal"]
            ))
    ...
    def visualize_nodes(self, output_path: Optional[str] = None) -> str:
        ...
        elements.append(Paragraph(
            ...
            self.styles["NavInstruction"]
        ))
        
        # Add summary statistics
        best_score_text = f"Best Validation Score: {self.node_manager.best_validation_score:.4f} (Node {self.node_manager.best_step})" \
                          if self.node_manager.best_validation_score > 0 else "No validation scores available"
        
        elements.append(Paragraph(
            f"Total Nodes: {len(all_nodes)} | {best_score_text}",
            self.styles["Normal"]
        ))
        elements.append(Spacer(1, 10))
        ...
        for node in all_nodes:
            ...
            toc_data.append([
                f"<a href='#node_{node.id}' color='blue'>{node.id}</a>",
                node.stage,
                status,
                node.tool_used or ""
            ])
