    ...
    while iteration < max_iterations:
        ...
        if success:
            ...
            if successive_failures >= max_successive_failures and best_success_found:
                logger.warning(f"Stopping search after {successive_failures} successive failures with a successful solution found")
                break
...
if __name__ == "__main__":
    ...
    parser.add_argument("-c", "--config", default=DEFAULT_CONFIG_PATH, help=f"YAML config file (default: {DEFAULT_CONFIG_PATH})")
    ...
    parser.add_argument("--enable-per-iteration-instruction", action="store_true", help="Enable per-iteration instruction")
