    ...
    def test_webui_ui_rendering(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            at.session_state["messages"].append(
                Message.user_summary("time limit: 5 mins", input_dir=test_input_dir)
            )
            ...
            at.session_state["messages"].append(
                Message.task_results(test_run_id, str(test_output_dir))
            )
            ...
            tabs = at.tabs
            assert len(tabs) > 0, f"No tabs found. Expected result tabs to be displayed."
